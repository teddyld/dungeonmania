...
public class BattleTest {
    ...
    @Test
    @Tag("11-23")
    @DisplayName("Test that a bribed mercenary applies an attack buff to the player")
    public void testSingleAllyAttackBuff() {
        DungeonManiaController controller = new DungeonManiaController();
        String config = "c_battleTests_allyBuff";
        DungeonResponse res = controller.newGame("d_battleTest_allyBuffSingle", config);
        ...
        // Player attack value (without ally buff)
        double playerBaseAttack = Double.parseDouble(TestUtils.getValueFromConfigFile("player_attack", config));

        // Ally attack buff value
    ...
    @Test
    @Tag("11-24")
    @DisplayName("Test that a bribed mercenary applies a defence buff to the player")
    public void testSingleAllyDefenceBuff() {
        DungeonManiaController controller = new DungeonManiaController();
        String config = "c_battleTests_allyBuff";
        DungeonResponse res = controller.newGame("d_battleTest_allyBuffSingle", config);
        ...
        double allyDefence = Integer.parseInt(TestUtils.getValueFromConfigFile("ally_defence", config));

        assertEquals((enemyAttack - allyDefence) / 10, -firstRound.getDeltaCharacterHealth(), 0.001);
    }

    @Test
    @Tag("11-25")
    @DisplayName("Test that multiple allies apply an additive attack and defence buff to the player")
    public void testMultipleAllyAttackBuff() {
        DungeonManiaController controller = new DungeonManiaController();
        String config = "c_battleTests_allyBuff";
        DungeonResponse res = controller.newGame("d_battleTest_allyBuffMultiple", config);


        // Pick up two treasures
        res = controller.tick(Direction.RIGHT);
        res = controller.tick(Direction.RIGHT);

        // Bribe the two mercenaries
        List<String> mercsIds = TestUtils.getEntitiesStream(res, "mercenary")
                                         .map(e -> e.getId())
                                         .collect(Collectors.toList());

        for (String id : mercsIds) {
            res = assertDoesNotThrow(() -> controller.interact(id));
        }
        ...
        assertEquals((playerBaseAttack + (allyAttack * 2)) / 5, -firstRound.getDeltaEnemyHealth(), 0.001);
        ...
        assertEquals((enemyAttack - (allyDefence * 2)) / 10, -firstRound.getDeltaCharacterHealth(), 0.001);
    }
}
